...
const PopupRoom = ({ popupInfo }) => {
  const { titulo, descripcion, precio, imagenes } = popupInfo;
    ...
    <Card sx={{ maxWidth: 400 }}>
      <ImageListItem sx={{ display: 'block' }}>
        <ImageListItemBar
          ...
          title={precio === 0 ? 'Free Stay' : '$' + precio}
          position="top"
        />
        <ImageListItemBar
          title={titulo}
          subtitle={descripcion.substr(0, 30) + '...'}
          sx={{ zIndex: 2 }}
        />
        <Swiper
          ...
          {imagenes.map((url) => (
            <SwiperSlide key={url}>
              <Box
                ...
                onClick={() =>
                  dispatch({ type: 'ACTUALIZA_CUARTO', payload: popupInfo })
                }
              />
            </SwiperSlide>
          ))}
